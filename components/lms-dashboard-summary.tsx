...
type Enrollment = { id: string; course_id: number; progress_percentage: number; enrollment_status: string };
type Course = { id: number; course_name: string; course_code: string };
    ...
    async function load() {
      try {
        ...
        const [enrollmentResult, courseResult, certificateResult] = await Promise.all([
          supabase.from("lms_enrollments").select("id,course_id,progress_percentage,enrollment_status").eq("student_id", user.id),
          supabase.from("lms_courses").select("id,course_name,course_code").eq("status", "Published"),
          supabase.from("lms_course_certificates").select("id", { count: "exact", head: true }).eq("student_id", user.id)
        ]);
