...
create table if not exists public.lms_course_assets (
  ...
  constraint lms_course_asset_type_check check (asset_type in ('Video', 'PDF Notes', 'PowerPoint', 'Assignment', 'Course Thumbnail')),
  constraint lms_course_asset_status_check check (asset_status in ('Draft', 'Published', 'Archived'))
);
...
with check (bucket_id = 'aff-course-assets' and lower(coalesce(auth.jwt() ->> 'email', '')) = 'acafffx@gmail.com');

create policy "AFF admin can update course storage"
on storage.objects for update to authenticated
using (bucket_id = 'aff-course-assets' and lower(coalesce(auth.jwt() ->> 'email', '')) = 'acafffx@gmail.com')
with check (bucket_id = 'aff-course-assets' and lower(coalesce(auth.jwt() ->> 'email', '')) = 'acafffx@gmail.com');
