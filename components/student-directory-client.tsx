...
export function StudentDirectoryClient() {
  ...
  const loadDirectory = useCallback(async () => {
    try {
      ...
      const response = await fetch("/api/admin/student-directory", { cache: "no-store" });
      const payload = await response.json().catch(() => ({}));

      if (!response.ok) {
        setStudents([]);
        const formattedError = payload?.error ? `${payload.code ? `Code: ${payload.code} | ` : ""}${payload.error}` : `Request failed with status ${response.status}`;
        setLoadError(formattedError);
        setMessage(`Unable to load student directory: ${formattedError}`);
        return;
      }
      const sortedStudents = ((payload.students ?? []) as DbRow[])
